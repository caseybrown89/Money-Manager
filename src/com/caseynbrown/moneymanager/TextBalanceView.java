	...
	public TextBalanceView(Context c, TextBalance tb){
		...
		if (! tb.getDate().equals("")){
			...
			if (tb.getLatestDate() != null){
				...
				String date = getResources().getString(R.string.balancelist_noentry);
				this.mLatest.setTypeface(null, Typeface.ITALIC);
				String test1 = this.mLatest.getText().toString();
				String test2 = this.mLatestAmount.getText().toString();
				this.mLatestAmount.setText("");
			}
		}
	}
